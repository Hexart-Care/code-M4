...
void tri_selection(int tab[], int tab2[], int taille, int ordre)
{
	...
	if (ordre == 0) {       //Trie ordre croissant des pouls
        ...
        for(int i = 0; i < taille; i++){
            res = maximum(tab2, taille2);
            swap(&tab2[res], &tab2[taille2]);   //Retrie les valeurs apres l'affichage sinon la recherche dichotomique ne fonctionnera plus
            swap(&tab[res], &tab[taille2]);
            taille2--;
        }
	}
...
int trie(int menu2,int tab[],int tab2[])
{
	...
	tri_selection(tab, tab2, taille, 3);
	}
	return 0;
}
...
{                                                           //Recherche dichotomique dasn des tableaux
    ...
    printf("la valeur %d n'est pas dans tableau\n", temps);
    }
...
int moyenne(int Tabpouls[],int Tabtemps[], int taille)
{
    ...
    printf("Entre quelles valeurs de temps voulez vous réalisez la moyenne des pouls ?\n");
